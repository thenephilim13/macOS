...
The variables **MLB, ROM, SystemSerialNumber, SystemUUID** have been censored in the config.plist because of security reasons. You can generate these variables by yourself and replace them in `config.plist`. Please find further documentation in the [Clover Vanilla Guide, RtVariables section](https://hackintosh.gitbook.io/-r-hackintosh-vanilla-desktop-guide/config.plist-per-hardware/coffee-lake#rtvariables-and-smbios). Make sure to use the model `iMac19,1`.
